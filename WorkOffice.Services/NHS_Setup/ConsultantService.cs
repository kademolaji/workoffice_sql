...
namespace WorkOffice.Services
{
    public class ConsultantService: IConsultantService
    {
        ...
        public async Task<ApiResponse<SearchReply<ConsultantViewModels>>> GetList(SearchCall<SearchParameter> options)
        {
            int count = 0;
            ...
            try
            {
                var apiResponse = new ApiResponse<SearchReply<ConsultantViewModels>>();
                ...
                var response = new SearchReply<ConsultantViewModels>()
                {
                    TotalCount = count,
                    Result = items.Select(x => x.ToModel<ConsultantViewModels>()).ToList(),
                };
            ...
            {
                return new ApiResponse<SearchReply<ConsultantViewModels>>() { StatusCode = System.Net.HttpStatusCode.BadRequest, ResponseType = new SearchReply<ConsultantViewModels>() { TotalCount = count }, IsSuccess = false };
            }
        }
        ...
        public async Task<ApiResponse<GetResponse<ConsultantViewModels>>> Get(long consultantId)
        {
            try
            {
                if (consultantId <= 0)
        ...
        public async Task<ApiResponse<DeleteReply>> Delete(long consultantId)
        {
            try
            {
                if (consultantId <= 0)
        ...
        public async Task<ApiResponse<CreateResponse>> Upload(byte[] record, long consultantId)
        {
            try
            {
                var apiResponse = new ApiResponse<CreateResponse>();
