...
namespace WorkOffice.Services
{
    public class RTTService: IRTTService
    {
        ...
        public RTTService(DataContext appContext, IAuditTrailService _auditTrail)
        {
            context = appContext;
            ...
            try
            {
                ...
                using (var trans = context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        entity = model.ToModel<RTT>();
                        context.RTTs.Add(entity);
                        ...
                        if (result)
                        {
                            ...
                            await auditTrail.SaveAuditTrail(details, "RTT", "Create");
                            trans.Commit();
                        }
                    }
                    catch (Exception ex)
        ...
        public async Task<ApiResponse<CreateResponse>> UpdateRTT(RTTViewModels model)
        {
            try
            {
                ...
                var apiResponse = new ApiResponse<CreateResponse>();
                bool result = false;
                var entity = await context.RTTs.FindAsync(model.RTTId);
                if (entity == null)
                {
                ...
                using (var trans = context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (result)
                        {
                            var details = $"Updated RTT: Code = {model.Code}, Name = {model.Name} ";
        ...
        public async Task<ApiResponse<SearchReply<RTTViewModels>>> GetList(SearchCall<SearchParameter> options)
        {
            int count = 0;
            ...
            try
            {
                var apiResponse = new ApiResponse<SearchReply<RTTViewModels>>();


                IQueryable<RTT> query = context.RTTs;
                ...
                var response = new SearchReply<RTTViewModels>()
                {
                    TotalCount = count,
                    Result = items.Select(x => x.ToModel<RTTViewModels>()).ToList(),
                };
            ...
            {
                return new ApiResponse<SearchReply<RTTViewModels>>() { StatusCode = System.Net.HttpStatusCode.BadRequest, ResponseType = new SearchReply<RTTViewModels>() { TotalCount = count }, IsSuccess = false };
            }
        }


        //public async Task<ApiResponse<GetResponse<List<RTTViewModels>>>> GetList(int pageNumber = 1, int pageSize = 10)
        //{
        //    try
        //    {
        //        var apiResponse = new ApiResponse<GetResponse<List<RTTViewModels>>>();
        ...
        //        if (items.Count <= 0)
        //        {
        //            return new ApiResponse<GetResponse<List<RTTViewModels>>>() { StatusCode = System.Net.HttpStatusCode.NotFound, ResponseType = new GetResponse<List<RTTViewModels>>() { Status = false, Message = "No record found" }, IsSuccess = false };
        //        }


        //        var response = new GetResponse<List<RTTViewModels>>()
        //        {
        //            Status = true,
        //            Entity = items.Select(x => x.ToModel<RTTViewModels>()).ToList(),
        //            Message = ""
        //        };
        ...
        //    catch (Exception ex)
        //    {
        //        return new ApiResponse<GetResponse<List<RTTViewModels>>>() { StatusCode = System.Net.HttpStatusCode.BadRequest, ResponseType = new GetResponse<List<RTTViewModels>>() { Status = false, Message = $"Error encountered {ex.Message}" }, IsSuccess = false };
        //    }
        //}
        ...
        public async Task<ApiResponse<GetResponse<RTTViewModels>>> Get(long rttId)
        {
            try
            {
                ...
                var apiResponse = new ApiResponse<GetResponse<RTTViewModels>>();

                var result = await context.RTTs.FindAsync(rttId);

                if (result == null)
                {
                    return new ApiResponse<GetResponse<RTTViewModels>>() { StatusCode = System.Net.HttpStatusCode.NotFound, ResponseType = new GetResponse<RTTViewModels>() { Status = false, Message = "No record found" }, IsSuccess = false };
                }

                var response = new GetResponse<RTTViewModels>()
                {
                    Status = true,
                    Entity = result.ToModel<RTTViewModels>(),
                    Message = ""
                };
            ...
            {
                return new ApiResponse<GetResponse<RTTViewModels>>() { StatusCode = System.Net.HttpStatusCode.BadRequest, ResponseType = new GetResponse<RTTViewModels>() { Status = false, Message = $"Error encountered {ex.Message}" }, IsSuccess = false };
            }
        }
        ...
        public async Task<ApiResponse<DeleteReply>> Delete(long rttId)
        {
            try
            {
                ...
                var details = $"Deleted RTT: Code = {result.Code}, Name = {result.Name} ";
                await auditTrail.SaveAuditTrail(details, "RTT", "Delete");
        ...
        public async Task<ApiResponse<DeleteReply>> MultipleDelete(MultipleDeleteModel model)
        {
            try
            {
                ...
                {
                    var data = await context.RTTs.FindAsync(item);
                    if (data != null)
                    {
                        data.IsDeleted = true;
                    }
                ...
                var details = $"Deleted Multiple RTT: with Ids {model.targetIds.ToArray()} ";
                await auditTrail.SaveAuditTrail(details, "RTT", "Delete");
        ...
        public async Task<ApiResponse<GetResponse<byte[]>>> Export()
        {
            try
            {
                ...
                var rtt = await (from a in context.RTTs
                                     where a.IsDeleted == false
                                     select new RTTViewModels
                                     {
                                         Code = a.Code,
                                         Name = a.Name,
                                     }).ToListAsync();
                ...
                if (rtt != null)
                {
                    ...
                    {
                        ExcelWorksheet ws = pck.Workbook.Worksheets.Add("RTT");
                ...
                var details = $"Downloaded RTT: TotalCount {rtt.Count} ";
                await auditTrail.SaveAuditTrail(details, "RTT", "Download");
        ...
        public async Task<ApiResponse<CreateResponse>> Upload(byte[] record, long rttId)
        {
            try
            {
                ...
                List<RTTViewModels> uploadedRecord = new List<RTTViewModels>();
                ...
                using (ExcelPackage excelPackage = new ExcelPackage(stream))
                {
                    ...
                    {
                        uploadedRecord.Add(new RTTViewModels
                        {
                ...
                var details = $"Uploaded Apptype: TotalCount {rtts.Count} ";
                await auditTrail.SaveAuditTrail(details, "RTT", "Upload");
