  ...
  implements OnInit
{
  ...
  editCall(row: { appointmentId: number }) {
    this.router.navigate(['nhs', 'edit-appointment', row.appointmentId]);
  }
  ...
  removeSelectedRows() {
    ...
    const targetIds = this.selection.selected.map((data) => data.appointmentId);
  ...
  public loadData(searchQuery: string, sortField: string, sortOrder: string) {
    ...
    this.appointmentService.getAllAppointment(options).subscribe((res) => {
