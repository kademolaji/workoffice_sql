...
export class AllStructureDefinitionComponent
  ...
  implements OnInit
{
  ...
  ELEMENT_DATA: StructureDefinitionService[] = [];
  isLoading = false;
  totalRows = 0;
  pageSize = 10;
  currentPage = 0;
  pageSizeOptions: number[] = [5, 10, 25, 100];
  dataSource: MatTableDataSource<StructureDefinitionModel> =
  ...
  removeSelectedRows() {
    ...
    const targetIds = this.selection.selected.map(
      (data) => data.structureDefinitionId
    );
    this.subs.sink = this.structureDefinitionService
      ...
      .subscribe({
        next: (res) => {
          if (res.status) {
            this.refresh();
            this.selection = new SelectionModel<StructureDefinitionModel>(
              true,
              []
            );
            this.showNotification(
  ...
  public loadData(searchQuery: string, sortField: string, sortOrder: string) {
    ...
    this.structureDefinitionService
      .getAllStructureDefinition(options)
      .subscribe((res) => {
