...
import { DeleteSpecialtyDialogComponent } from './dialog/delete/delete.component';
  ...
  implements OnInit
{
  ...
  selection = new SelectionModel<SpecialtyModel>(true, []);
  ELEMENT_DATA: SpecialtyService[] = [];
  isLoading = false;
  totalRows = 0;
  pageSize = 10;
  currentPage = 0;
  pageSizeOptions: number[] = [5, 10, 25, 100];
  dataSource: MatTableDataSource<SpecialtyModel> =
  ...
  addNew() {
    this.router.navigate(['setup', 'specialty', 'add-specialty']);
  }

  editCall(row: { specialtyId: number }) {
  ...
  removeSelectedRows() {
    ...
    const targetIds = this.selection.selected.map(
      (data) => data.specialtyId
    );
    this.subs.sink = this.SpecialtyService
      .deleteMultipleSpecialty(targetIds)
      .subscribe({
        next: (res) => {
          if (res.status) {
            this.refresh();
            this.selection = new SelectionModel<SpecialtyModel>(
              true,
              []
            );
            this.showNotification(
