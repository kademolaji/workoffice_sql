...
import { DeleteWardDialogComponent } from './dialog/delete/delete.component';

@Component({
  selector: 'app-all-ward',
  templateUrl: './all-ward.component.html',
  styleUrls: ['./all-ward.component.css'],
})
export class AllWardComponent
  ...
  implements OnInit
{
  ...
  ELEMENT_DATA: WardService[] = [];
  isLoading = false;
  totalRows = 0;
  pageSize = 10;
  currentPage = 0;
  pageSizeOptions: number[] = [5, 10, 25, 100];
  dataSource: MatTableDataSource<WardModel> =
  ...
  addNew() {
    this.router.navigate(['setup', 'ward', 'add-ward']);
  }

  editCall(row: { wardId: number }) {
  ...
  removeSelectedRows() {
    ...
    this.subs.sink = this.WardService
      .deleteMultipleWard(targetIds)
      .subscribe({
        next: (res) => {
          if (res.status) {
            this.refresh();
            this.selection = new SelectionModel<WardModel>(
              true,
              []
            );
            this.showNotification(
