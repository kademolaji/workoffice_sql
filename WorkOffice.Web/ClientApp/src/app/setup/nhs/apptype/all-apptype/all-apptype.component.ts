...
import { DeleteAppTypeDialogComponent } from './dialog/delete/delete.component';

@Component({
  selector: 'app-all-apptype',
  templateUrl: './all-apptype.component.html',
  styleUrls: ['./all-apptype.component.css'],
})
export class AllAppTypeComponent
  ...
  implements OnInit
{
  ...
  selection = new SelectionModel<AppTypeModel>(true, []);
  ELEMENT_DATA: AppTypeService[] = [];
  isLoading = false;
  totalRows = 0;
  pageSize = 10;
  currentPage = 0;
  pageSizeOptions: number[] = [5, 10, 25, 100];
  dataSource: MatTableDataSource<AppTypeModel> =
    ...
    this.subs.sink = this.AppTypeService
      ...
      .subscribe({
        next: (res) => {
          if (res.status) {
            this.refresh();
            this.selection = new SelectionModel<AppTypeModel>(
              true,
              []
            );
            this.showNotification(
